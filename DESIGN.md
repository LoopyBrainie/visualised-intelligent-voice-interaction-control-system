# Design System: Apple × Morandi Hybrid

## Design Philosophy

This is a **hybrid design system** combining:

- **Apple** design principles: color palette, typography, card shadows, accessibility focus rings
- **iOS 26 Morandi** aesthetics: Web Animation API spring effects, glassmorphism, warm ambient atmosphere

The result is "Apple's bones with Morandi's flesh" — the structural clarity of Apple design with the sensory depth of iOS 26 Morandi.
...
- Apple card shadow formula: `3px 5px 30px rgba(0, 0, 0, 0.22)`
- 2px solid focus rings for accessibility

**From Morandi (retained):**

- Warm Morandi background (`#F2EFE9` light / `#1A1918` dark) instead of Apple binary black/gray
- `backdrop-filter: blur(30px) saturate(140%)` glassmorphism on cards and modals
- Web Animation API spring press effects on buttons
...
- Apple Blue (`#0071e3`) as sole chromatic accent
- Warm Morandi backgrounds (not Apple pure black/gray alternation)
- Glassmorphism cards with Apple shadow formula
- WAAPI spring animations for tactile button feedback
- No gradients on backgrounds (pure color fields)
- Compact headline line-heights (1.07) for Apple typography feel
- 8px radius on primary CTAs (not 980px pills — those reserved for text links)
